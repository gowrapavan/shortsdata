...
async def fetch_hoofoot():
    ...
    if os.path.exists(FILENAME):
        try:
            ...
            print(f"📖 Loaded {len(existing_urls)} existing matches.")
        except: pass

    results = existing_data

    async with async_playwright() as p:
        # Added --disable-blink-features to hide automation
            ...
            try:
                ...
                if player:
                    ...
                    if a_tag: embed_url = a_tag["href"]
                        ...
                        if iframe: embed_url = iframe["src"]

                if embed_url:
                    results.append({**m, "embed_url": embed_url})
                    # Save every time so we don't lose data
